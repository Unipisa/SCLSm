...
let Multinomial (n: int) (ni: int array) = floor (0.5 + exp ((FactorialLn n) - (Array.fold (fun acc a -> acc + (FactorialLn a)) 0.0 ni)))
...
let CumulativeSum (arr: float array) = Array.scanReduceBack (fun acc x -> acc + x) arr
...
let Histogram (X: float []) n =
    ...
    let xmin = Array.reduce (fun a b -> min a b) X
    let xmax = Array.reduce (fun a b -> max a b) X
    // Create an array of means.
    let segs = Array.init (n+1) (fun i -> if i = 0 then xmin
                                          elif i = n then xmax
                                          else xmin + (float i) * (xmax - xmin) / (float n))
    // Count the number of elements in each bin.
    Array.iter (fun a -> let segfind = Array.tryFindIndex (fun x -> x > a) segs
                         let segidx = if segfind.IsNone then n-1 else (segfind.Value) - 1
                         //printf "%d\n" segidx
                         H.[segidx] <- H.[segidx] + 1
                         ()) X
    H, segs